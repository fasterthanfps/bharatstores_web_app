// lib/scraper/stores/dostana.ts
// dostana-foods.com — Indian grocery store in Germany. Shopify store with suggest.json API.

import { BaseScraper, ScrapedListing, ScraperResult } from '../base';
import { findCategoryUrl } from '../categoryMap';

export class DostanaScraper extends BaseScraper {
    storeId = 'dostana';
    storeName = 'Dostana';
    private baseUrl = 'https://dostana-foods.com';
    ...
    async scrape(query: string): Promise<ScraperResult> {
        ...
        // Dostana's Shopify endpoints. We try both root and English language route
        const endpoints = [
            `${this.baseUrl}/search/suggest.json?q=${encodeURIComponent(query)}&resources[type]=product&resources[limit]=20`,
            `${this.baseUrl}/en/search/suggest.json?q=${encodeURIComponent(query)}&resources[type]=product&resources[limit]=20`,
        ...
        for (const url of endpoints) {
            try {
                ...
                for (const item of products.slice(0, 20)) {
                    ...
                    const priceRaw = item.price || item.variants?.[0]?.price || '0';
                    let price = 0;
                ...
                errors.push(`Dostana API error for URL ${url}: ${e.message}`);
            }
        }

        // Category fallback
        if (listings.length === 0) {
            const catPath = findCategoryUrl(q, 'dostana');
            if (catPath) {
                try {
                    ...
                    errors.push(`Dostana category fallback error: ${e.message}`);
                }
            }
        }
